...
/* With some extensions/changes by E.Schriek */
...
namespace tm
{
    ...
    public class SigScan
    {
        ...
        public SigScan()
        {
            this.m_processHandle = IntPtr.Zero;
        ...
        public SigScan(IntPtr processHandle, IntPtr addr, int size)
        {
        ...
        private bool DumpMemory()
        {
            try
            {
                ...
                var ret = ReadProcessMemory(
                    this.m_processHandle, this.m_vAddress, this.m_vDumpedRegion, this.m_vSize, out nBytesRead
                    );
        ...
        {
            List<IntPtr> foundPatterns = new List<IntPtr>();
            ...
            try
            {
                ...
                for (int x = 0; x < this.m_vDumpedRegion.Length; x++)
                {
                    if (this.MaskCheck(x, btPattern, strMask))
                    {
                        ...
                        foundPatterns.Add(new IntPtr((Int64)this.m_vAddress + (x + nOffset)));
                    }
                }
